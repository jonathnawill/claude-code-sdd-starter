...
4. Monte a descrição do PR usando `.claude/templates/pr-template.md`.
5. Siga o processo padrão de git/PR: revisar `git status`/`git diff`, confirmar que não há arquivos indevidos staged, criar commit(s) com mensagens claras, e abrir o PR — **sempre pedindo confirmação do usuário antes de dar push ou abrir o PR**, conforme as regras de ações de risco deste ambiente.
6. Não faça merge automático. Abrir o PR é o fim desta etapa; merge é decisão humana.
